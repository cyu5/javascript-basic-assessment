...
const getValueOfProperty = (obj, propertyName) => obj[propertyName];
...
const makeChatMessage = (msg, author) => {
	let chatMessage = {
		message: msg,
		author: author,
		timestamp: new Date()
...
// and then changes the person object to have a property called lovesCode with a value of 10.  
// If their name is Brack set lovesCode to 0.  otherwise set lovesCode to 5.
const coderTest = personObj => {
...
const outside = (temperature, humidity, cloudiness) => {
	...
	let message = "I love outside";

	if (temperature > 80) 
	{
		if (humidity > 40)
		{
			message = "I'm all sweat";
		}
		else if (humidity < 40 && cloudiness < 20)
		{
			message = "I'm literally in the desert";
		}
		else
		{
			message = "Hmm, probably not";
		}
	}
	else if (temperature < 40 && cloudiness > 40)
	{
		message = "I have icecicles";
	}
	else if (humidity > 50 || cloudiness > 50)
	{
		message = "Hmm, probably not";
	}
...
const callerBack = (holla, back) => holla(back + ' back');
